...
- **Режим размышления.** Qwen3 умеет думать вслух и оборачивает рассуждения в `<think>…</think>`. Блоки вырезаются из потока и уходят отдельным SSE-событием `think`: на экране они показываются сворачиваемым блоком над ответом, **в базу не пишутся и в контекст следующего запроса не подмешиваются**. Тумблер в чате виден только у моделей из списка `thinking_models` в настройках; когда он выключен, к сообщению в контексте (не в базе) дописывается `/no_think`.
