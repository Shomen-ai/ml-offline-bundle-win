...
- **Выбор модели** хранится на диалоге (`dialogs.model_name`): у разных диалогов — разные модели; при отправке сообщения нужная грузится автоматически (первый ответ после переключения ждёт загрузку).
...
| GET | `/api/models` | список моделей LLM-сервера |
| GET/POST | `/api/dialogs` | список / создать диалог |
| PATCH/DELETE | `/api/dialogs/{id}` | переименовать, сменить модель / удалить |
