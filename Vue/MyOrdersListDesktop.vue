<template>
  <div class="w-full">
    ...
    <!-- ================== МОБИЛЬНЫЙ ВИД ================== -->
    <!--
      Сюда вставляется существующий мобильный блок из MyOrdersTableNew.vue
      (тот, что был под v-else). Слот получает уже отфильтрованные данные,
      но можно и просто положить внутрь свою готовую разметку.
    -->
    <div v-else class="w-full">
      <slot
        ...
        :customers="customers"
        :columns="columns"
        :filtered="h.filteredCustomers.value"
      >
        <!-- запасной вид, если слот не передали -->
...
import { useDevice } from '@/components/Composables/useDevice'
